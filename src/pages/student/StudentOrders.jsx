...
export default function StudentOrders() {
  ...
  const filtered = orders.filter(o => {
    const matchesTab =
      ...
      o.status === tab;
    const matchesSearch = !search || o.orderId?.toLowerCase().includes(search.toLowerCase()) || o.shop?.shopName?.toLowerCase().includes(search.toLowerCase());
                  ...
                  <Card className="p-5 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-gray-800 truncate">{order.file?.fileName}</p>
                        <p className="text-sm text-gray-500 mt-1">{order.shop?.shopName}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{formatDate(order.createdAt)}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="font-bold text-blue-700">{formatCurrency(order.pricing?.grandTotal)}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{order.printSettings?.copies} cop{order.printSettings?.copies > 1 ? 'ies' : 'y'}</p>
